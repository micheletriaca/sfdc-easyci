...
function computeDelta (metadata, originalPackageXml) {
  ...
  if (staticResourceTag && staticResourceTag.members === '*') {
...
function restoreOriginalPackageXml (originalPackageXml) {
  ...
  return fileUtils.readPackageXml('./src/package.xml', false)
    ...
    .then(mergedPackage => {
      return new Promise((resolve, reject) => {
        var builder = new xml2jsx.Builder({
          renderOpts: {
            'pretty': true,
            'indent': '    ',
            'newline': '\n'
        ...
        var xml = builder.buildObject(mergedPackage) + '\n'

        fs.writeFile('./src/package.xml', xml, function (err, data) {
          if (err) reject(err)
  ...
  return {
    retrieve () {
      ...
      fileUtils
        ...
        .then(() => computeDelta(tmpMetadata, tmpOriginalPackageXml))
