...
function computeDeltaAndSyncDeleted (metadata, originalPackageXml, packagexmlPath) {
  ...
  return fileUtils.deleteFilesInDirectory(packagexmlPath + 'staticresources', fileShouldBeDeleted).then(() => deltaPackageXml)
}

function restoreOriginalPackageXml (originalPackageXml, packagexmlPath) {
  ...
  return xmlUtils.processXml(packagexmlPath, packageToMerge => {
    let pRoot = packageToMerge.Package
    let staticResourceTag = _.find(pRoot.types, {name: 'StaticResource'})
...
module.exports = function (conn, packagexmlPath) {
  ...
  return {
    retrieve () {
      status.start()
      xmlUtils
        .readPackageXml(packagexmlPath + 'package.xml')
        ...
        .then(() => computeDeltaAndSyncDeleted(tmpMetadata, tmpOriginalPackageXml, packagexmlPath))
        ...
        .then(data => fileUtils.extractZipContents(data, packagexmlPath))
        .then(() => restoreOriginalPackageXml(tmpOriginalPackageXml, packagexmlPath))
        .then(() => persistNewMetadataConfig(tmpMetadata))
        .then(() => removeUselessFLS(packagexmlPath + 'profiles'))
        .then(() => removeUselessFLS(packagexmlPath + 'permissionsets', false))
        .then(() => removeUselessTranslations(packagexmlPath + 'objectTranslations'))
        .then(f => {
          status.stop()
          console.log('\n' + chalk.green('Done'))
        })
