...
function removeUselessFLS (profileDir, defaultEnabled = true) {
  fileUtils.readDirectory(profileDir)
    .then(profiles => {
      profiles.forEach(p => xmlUtils.parseXml(profileDir + '/' + p)
        .then(jsonProfile => {
          const rootKey = _.keys(jsonProfile)[0]
          jsonProfile[rootKey].fieldPermissions = _.filter(jsonProfile[rootKey].fieldPermissions, fp => {
            return fp.editable === !defaultEnabled + '' || fp.readable === !defaultEnabled + ''
          })
          return jsonProfile
        })
        .then(jsonProfile => xmlUtils.buildAndStoreXml(jsonProfile, profileDir + '/' + p))
      )
    })
}
