...
function computeDeltaAndSyncDeleted (metadata, originalPackageXml) {
  ...
  return fileUtils.deleteFilesInDirectory('./src/staticresources', fileShouldBeDeleted).then(() => deltaPackageXml)
}
...
function restoreOriginalPackageXml (originalPackageXml) {
  ...
  return xmlUtils.processXml('./src/package.xml', packageToMerge => {
...
function removeUselessFLS (profileDir, defaultEnabled = true) {
  xmlUtils.processXmlsInDirectory(profileDir, jsonProfile => {
    ...
    pRoot.fieldPermissions = _.filter(pRoot.fieldPermissions, fp => {
      return fp.editable === !defaultEnabled + '' || fp.readable === !defaultEnabled + ''
    })
    return jsonProfile
  })
}
  ...
  return {
    retrieve () {
      ...
      xmlUtils
        ...
        .then(() => removeUselessFLS('./src/profiles'))
        .then(() => removeUselessFLS('./src/permissionsets', false))
        .then(() => removeUselessTranslations('./src/objectTranslations'))
        .then(f => {
          status.stop()
          console.log('\n' + chalk.green('Done'))
        })
