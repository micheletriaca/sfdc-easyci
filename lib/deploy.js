...
function restoreUselessFLS (profileDir, defaultToRestoreIsRw = true) {
  // 1st step: get field infos from objects
  // 2nd step: rebuild missing FLSs, except for required fields
  return xmlUtils.processXmlsInDirectory(profileDir, (jsonProfile, profileName, done) => {
    ...
    xmlUtils.readXmlsInDirectory('./src/objects', (jsonObject, fileName) => {
...
function restoreDisabledPermissions (profileDir) {
  xmlUtils
    ...
    .then(adminPermissions => xmlUtils.processXmlsInDirectory(profileDir, (jsonProfile, profileName) => {
      let currentProfileUserPermissions = _.groupBy(jsonProfile.Profile.userPermissions, 'name')
      _.each(adminPermissions, ap => {
